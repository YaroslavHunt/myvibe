...
export class WinstonLoggerService implements LoggerService {
	private static readonly levels = ['info', 'warn', 'error', 'fatal'];
	private static readonly localTimestamp = () => {
		const date = new Date();
			...
			new winston.transports.Console({
				format: winston.format.combine(
					...
					winston.format.timestamp({
						format: WinstonLoggerService.localTimestamp
					}),
					winston.format.printf(
						({ timestamp, level, message, stack, context }) => {
							const ctx = context ? `[${context}]` : '[App]';
							const coloredCtx = winston.format
								.colorize()
								.colorize('label', ctx);
